...
$(document).ready(function () {
    ...
    function checkUpdates() {
        ...
        var newBuild = buildNumber;
        if (newUpdate != ver) ver = 'v' + ver + ' (Update Available v' + latestVersion + ')';
        else if(newBuild != build)  ver = 'v' + ver + ' (Update Available v' + latestVersion + ' build ' + buildNumber + ')';
        else ver = 'v' + ver;
    }

    function optionsWindow() {

        var globalCSS = '<style>#nav{padding: 15px 0 15px 10px;}#textNum{text-align: center;padding: 0;margin: 0;}div.alertbox {border:0;}input[type="button"] {border: none;cursor: pointer;padding: 5px 40px;font-weight: bold;}input[type="submit"] {border: none;cursor: pointer;padding: 7px 25px;font-weight: bold;background: #689F38;color: #fff;}</style>';
    ...
    $('.rdRefreshItem').change(function () {
        ...
        if (val == '15') {
            GM_setValue('RefreshItemsAndBookmarks', '1');
            GM_setValue('countDown', '15');
        } else if (val == '30') {
            GM_setValue('RefreshItemsAndBookmarks', '2');
